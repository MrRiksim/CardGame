...
## Prerequisites:
Download and install Java, Maven and IntelliJ:\
...
## How to run:
Turn on IntelliJ, go to `File > Open` and click to open the pom.xml file in either server or client directory (they have to be opened separately). Run `GameServer.java` for a server instance or `GameClient.java` for client instances.\
